...
代码中call rd_disk_m_16是将加载程序加载到LOADER_BASE_ADDER处，即0x0900处，至于为什么要将加载程序起始地址放在0x0900处？

原因是：

- 首先， MBR受限于512字节大小，在这么小的空间内，代码部分只有446字节，因此无法位内核准备好环境，更没发将内核成功加载到内存并运行。所以需要在另一个程序中完成初始化环境及加载内核的任务，这个程序也就是loader，所以要另外找一个地方存储。而在实模式下0x0500～0x7BFF和0x7E00～0x9FBFF这两段内存都是可用区域，这里就选择了0x0900

- 其次，loader中要定义一些数据结构（如GDT等），这些数据结构将来内核还会用到，所以loader加载到内核之后不能覆盖。随着功能不断添加，内核会越来越大，其所在的内存地址也会越来越向高地址发展，因此这里把loader尽量放在低地址。



## 添加一个loader程序

之前读取硬盘第2扇区（从0开始数），但是该扇区中并未放入具体loader程序，所以无法看出居图效果，这里添加一个在屏幕打印“2 loader”字符串的程序。
...
使用nasm编译后将得到的loader.bin通过dd命令写入到硬盘中：

```shell
dd if=/xx/loader.bin of=/xx/hd60M.img bs=512 count=1 seek=2 conv=notrunc
```

这里seek=2是从0开始跳过两个扇区，即跳过第0和第1扇区写入第2扇区。

## 总结

在《操作系统真象还原》这本书的初期里，直接从MBR中加载loader程序，并没有像现在的计算机系统那样，从MBR先搜索区可引导分区，再从可引导分区中读区分区引导扇区，最后从分区引导扇区中加载loader程序。

可能是因为现在的计算机设计为了可以使用多种系统，所以不能直接把加载程序限制死，不同的操作系统对应不同的加载程序。虽然不能将加载程序限制死，但是可以把加载程序的入口即MBR限制死，MBR是和操作系统无关的。硬盘相应分区的第一个扇区是和操作系统相关，操作系统的引导由它来完成。

而我们编写的MBR简化了分区引导扇区的搜索与检验，直接加载laoder程序。

再次思考，其实分区引导扇区中就存放着loader程序，加载分区引导扇区就相当于加载laoder程序，只不过出于简单的原因我们这里直接指定了分区引导扇区就是第2个扇区，不像现在计算机系统那样有很多选择。所以也就省去了搜索以及检验的步骤。